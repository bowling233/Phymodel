...
现代科学研究常以数学、物理等基础学科作为理论基础，而将计算机作为数值运算和可视化工具。在本次研究性学习中，我们选取物理学的经典模型——刚性球体碰撞系统为研究对象，研究在计算机上模拟该模型所需的程序设计、算法优化和可视化等内容，并将该模型应用到实际问题上。
...
在现代科学研究中，计算机应用已经成为不可或缺的一环。计算机凭借其强大的性能和高效的算法，能够快速处理海量数据并得到准确的结果。利用可视化技术，研究人员得以实时观察数据情况，获得直观感受。

### 1.2 研究目的与意义

通过此次研究，我们希望深入探索小球碰撞这一经典模型，学习物理研究的基本方法；探索在计算机上模拟物理过程，学习使用算法优化程序流程、处理大量数据；学习使用图形库将研究结果可视化；学习运用科学研究方法探索、分析并解决问题，实际体验科学研究过程。这将极大地培养我们的和科学研究能力，为我们将来从事科学研究工作打下坚实基础。

### 1.3 研究内容

物理：刚性小球碰撞模型

计算机科学与技术：面向对象程序设计；排序算法（基于堆的优先队列）；数据结构（使用八叉树划分空间区域）；可视化（使用OpenGL和C++）
...
本章简要介绍本研究性学习所使用的物理模型：首先对刚性球体碰撞建模，然后分析系统内元素间相互作用过程，最后介绍该模型的具体应用场景。
...
* 实体及其状态的物理模型：
  ...
  * 长方体容器：容器中心的位置$\vec{p}$，容器的长宽高$a,b,c$。

系统中只有小球运动，小球与其它物体的相互作用（碰撞）可以分成三类问题：碰撞预测、检测和处理。所有碰撞均为弹性碰撞，不考虑摩擦和自旋。

物理过程的分析方法将决定编程模型：时间驱动模型依赖于碰撞检测，事件驱动模型依赖于碰撞预测。各类物体的碰撞检测都已有非常详尽的研究，可以直接采用。本论文主要讨论了事件驱动模型，使用碰撞预测分析。
...
在这一章，我们主要使用UML(Unified Modeling Language，统一建模语言)来描述我们的程序设计过程。我们采取面向对象方法、自顶向下的思路来设计程序，再用自底向上方法开发程序，一步步实现整个系统。

### 面向对象分析

为了使用计算机实现对上述物理过程的模拟，并将其运用到更多场景中，我们这些物理模型抽象为程序中的对象，物理过程抽象为这些对象的操作。我们将设计一个碰撞系统对象，它的用户视图如图3.1所示：

* 碰撞系统（CollisionSystem）对象：
  * 属性：记录系统中所有物体的状态
  * 操作：输入数据、更新系统状态、显示系统状态

> 图片3.1 用户视图

按第二章物理分析的思路，我们在碰撞系统内创建实体（Object）类和事件（Event）类。实体类保存实体信息并提供实体相关操作（如碰撞处预测、处理等）。事件类保存碰撞事件的信息，并提供对事件的处理（在实现中，处理会转交给具体的实体）。图3-2给出了该碰撞系统的类图，列出了它们的属性和方法。
...
在实现数据类型前应当设计好API（应用程序接口）。下文展示了CollisionSystem类、Event类和Ball类的API。其中，空间向量使用GLM（OpenGL Mathematics）库的vec3类来表示，该库为OpenGL和图形学量身定制，提供了对向量、矩阵等运算的支持。
...
在实现Ball::bounce()函数时，出现了类继承的问题。在Event类型中，我们只储存第二碰撞物体的基类指针（这是因为：如果为每一种继承类的物体都创建指针用于保存，将使Event类与Object类及其继承体系不能相互独立，影响程序的扩展开发。如果开发了新的Object子类型，那么Event的实现也要作出相应的更改）。在调用Event::handle()函数时，该函数无法确定调用Object::bounce()的哪个版本。最终，我们为Object类实现了一个统一的type接口，提供对象的具体子类型，然后使用动态绑定dynamic_cast来转换引用类型。
...
已知旋转前法向量为$\vec{b}$（before），旋转后法向量为$\vec{a}$（after），可以得出旋转角$\theta$和旋转轴$\vec{k}$：
